#!/usr/bin/env python3
"""
MintU Backend API Comprehensive Testing - Final Review (Fixed Validation)
...
class MintUComprehensiveTester:
    ...
    def test_smart_alerts(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Can be either a list or dict with alerts field
                if isinstance(data, list) or (isinstance(data, dict) and "alerts" in data):
                    alert_count = len(data) if isinstance(data, list) else len(data.get("alerts", []))
                    self.log(f"✅ Smart alerts - Count: {alert_count}", "PASS")
                    self.add_result("GET /api/alerts/smart", True, f"{alert_count} alerts")
                    return True
                else:
                    self.log(f"❌ Unexpected response format: {type(data)}", "FAIL")
    ...
    def test_money_school_daily(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                if "lesson" in data or "title" in data or "content" in data or "message" in data:
    ...
    def test_ai_agents(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Can be either a list or dict with agents field
                if isinstance(data, list) and len(data) > 0:
                    ...
                    self.log(f"✅ AI Agents - Count: {agent_count}", "PASS")
                    self.add_result("GET /api/ai/agents", True, f"{agent_count} agents available")
                    return True
                else:
                    self.log(f"❌ Expected non-empty list or agents dict, got: {data}", "FAIL")
    ...
    def test_ai_proactive_nudges(self):
        ...
        try:
            ...
            if response.status_code == 200:
                data = response.json()
                # Can be either a list or dict with nudges field
                if isinstance(data, list):
                    self.log(f"✅ Proactive nudges - Count: {len(data)}", "PASS")
                    self.add_result("GET /api/ai/proactive-nudges", True, f"{len(data)} nudges")
                    return True
                elif isinstance(data, dict) and "nudges" in data:
                    nudge_count = len(data["nudges"])
                    self.log(f"✅ Proactive nudges - Count: {nudge_count}", "PASS")
                    self.add_result("GET /api/ai/proactive-nudges", True, f"{nudge_count} nudges")
                    return True
                else:
                    self.log(f"❌ Unexpected response format: {type(data)}", "FAIL")
